...
function CemberinAlani(yaricap2, pi) {
  var pi = 3.14159;
  const alanHesapla = pi * Math.pow(yaricap2, 2);
  return alanHesapla;
}
...
var sayiAdedi = sayilar.length;
console.log("Sayılar dizisindeki sayıların toplam elemanı:", sayiAdedi);
...
for (let i = 1; i < sayilar.length; i++) {
  if (sayilar[i] > enbuyuk) {
    enbuyuk = sayilar[i];
  } else if (sayilar[i] < enkucuk) enkucuk = sayilar[i];
}
...
var ucebolunenlerintoplami = ucetambolunenler.reduce(function (total, sayii) {
  return total + sayii;
}, 0);
